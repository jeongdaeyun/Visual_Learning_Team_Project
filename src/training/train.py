...
from open_clip import ClipLoss, ClipLoss2
...
# 짝짓기 함수
def find_similar_pairs(features):
    num_features = features.size(0)
    pairs = []

    # 모든 쌍의 코사인 유사도 계산
    for i in range(num_features):
        best_similarity = -1  # 최소 유사도 값으로 초기화
        best_match = -1
        for j in range(num_features):
            if i != j:  # 자기 자신 제외
                ...
                if similarity > best_similarity:
                    ...
                    best_match = j
        pairs.append((i, best_match, best_similarity))  # (현재 인덱스, 가장 유사한 인덱스, 유사도 값)
...
def create_negative_from_tensor(tensor_batch):
    batch_size = tensor_batch.shape[0]
    new_tensor_batch = tensor_batch.clone()

    for i in range(batch_size // 2):  
        pos_index = i  
        neg_index = i + (batch_size // 2) 

        tokens = tensor_batch[pos_index].tolist()
        start_token, end_token = tokens[0], tokens[-1]

        content_tokens = tokens[1:-1]
        content_tokens = [token for token in content_tokens if token != 0]
        last_token = content_tokens[-1] if content_tokens else None
        content_tokens = content_tokens[:-1] if last_token else content_tokens

        if len(content_tokens) >= 2:
            idx1, idx2 = random.sample(range(len(content_tokens)), 2)
            content_tokens[idx1], content_tokens[idx2] = content_tokens[idx2], content_tokens[idx1]
        if last_token:
            content_tokens.append(last_token)

        negative_tokens = [start_token] + content_tokens + [end_token]
        
        while len(negative_tokens) < len(tokens):
            negative_tokens.append(0)
        # print(neg_index)
        # print(negative_tokens)
        # sys.exit()
        new_tensor_batch[neg_index] = torch.tensor(negative_tokens, device=tensor_batch.device)

    return new_tensor_batch
...
def data_wrapper(args, batch):
    if "CC3M" in args.train_data:
        ...
        images, texts, info_dict, pos = batch    # images, texts, neg, pos 
        # info_dict = {}
        mil_texts = None
...
def train_one_epoch(
    ...
    for i, batch in enumerate(dataloader):
        ...
        ) = data_wrapper(args, batch)   #### 터지기 시작하는 부분  text...?
        ...
        with autocast():
            ...
            # # # neg_loss2 = 0
            # # # # print(texts)
            negative_texts = create_negative_from_tensor(texts)
            # # # # # sys.exit()
            image_features2, text_features2, logit_scale2 = model(images, negative_texts)
            ...
            loss2_weight = 1.0
            total_loss += (loss2_weight * neg_loss2)
        ...
        if is_master(args) and (
            ...
            # loss_neg_m.update(loss_neg.item(), batch_size)  ###print
            logit_scale_scalar = logit_scale.item()
            
            logging.info(
                f"Train Epoch: {epoch} [{num_samples:>{sample_digits}}/{samples_per_epoch} ({percent_complete:.0f}%)] "
                f"Loss: {loss_m.val:#.5g} ({loss_m.avg:#.4g}) "
                # f"Loss Neg: {loss_neg_m.val:#.5g} ({loss_neg_m.avg:#.4g}) " ###print
