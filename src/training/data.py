...
def choose_negs_function(args):
    ...
    elif args.neg_type == "rand_both": ## NegativeAuto의 spacy- en_core_web_sm에서 에러 발생
...
class CsvDataset(Dataset):
    def __init__(
        self, input_filename, transforms, img_key, caption_key, negative_key, sep="\t", args=None
    ):
        ...
        self.negative_captions = df[negative_key].tolist()
        self.transforms = transforms
        self.root_dir = os.path.dirname(input_filename)
        self.args = args
        if self.args.use_only_quality_captions:    ### json 파일로 저장됨 negs_and_pos.py quality_caption_class() create_ca()로 만들어짐
        ...
        if self.args.mil_dense: 
            ###### self.image 경로 수정 & images with mil full name도 
            self.images = ["training/" + os.path.splitext(os.path.basename(s))[0] for s in self.images] 
            images_with_mil = os.listdir(self.args.mil_dense)
            images_with_mil_full_name = [
                "training/" + s.strip(".jpg.txt") for s in images_with_mil
            ]
        ...
        # self.negs = choose_negs_function(args) #negative aug 함수
    ...
    def __getitem__(self, idx):
        ...
        if self.args.vl_negs:  #True
            negatives =   tokenize([str(self.negative_captions[idx])])[0]     # self.negs.create_negs(self.captions[idx])
        ...
        if self.args.vl_pos or self.args.vl_negs:
            if self.args.mil_dense:
                ...
                return images, texts, info_dict,  positive ### 현재 디버깅 args로는 여기 해당
...
def get_csv_dataset(args, preprocess_fn, is_train, epoch=0):  ######## 손봐야 되는 파트 #########
    ...
    dataset = CsvDataset(
        ...
        negative_key= args.csv_negative_key,
