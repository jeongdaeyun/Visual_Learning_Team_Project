...
import sys
sys.path.append('/home/sliver/daeyun/github_code/src') # please check your root
import numpy as np
import torch
...
def main():
    ...
    device = init_distributed_device(args) #### gpu 분산 학습 설정
    # device  = "cuda:1"
    ...
    # initialize datasets
    data = get_data(args, (preprocess_train, preprocess_val), epoch=start_epoch)   ####### 데이터 가져오기
    assert len(data), "At least one train or eval dataset must be specified."

    # create scheduler if train 
    ## 학습에 필요한 scheduler 생성
    scheduler = None
    ...
    torch.save(
        checkpoint_dict,
        os.path.join(args.logs, f"eval_epoch_{start_epoch}.pt"),
    )
    print(
        f'saved in {os.path.join(args.logs, f"eval_epoch_{start_epoch}.pt")}'
    )
        # return
    
    if args.eval_only:
        if args.eval_vl_cklist:
            EvaluateAllVL(model, preprocess_val, start_epoch, args, writer)   ## Evaluate 파트
        return
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if args.save_logs:
            ...
            if completed_epoch == args.epochs or (
                ...
                torch.save(
                    checkpoint_dict,
                    os.path.join(args.logs, f"epoch_{completed_epoch}.pt"),
                )
            
            torch.save(
                checkpoint_dict,
                os.path.join(args.logs, f"epoch_latest_{completed_epoch}.pt"),
            )
