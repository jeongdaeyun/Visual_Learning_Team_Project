...
class ClipLoss(nn.Module):
    ...
    def forward(
        ...
        # labels2 = torch.arange(text_features.shape[0], device=device, dtype=torch.long)  # [0, 1, ..., 19]
...
class ClipLoss2(nn.Module):
    ...
    def forward(
        ...
        # 분리: Positive Text Features와 Negative Text Features
        num_positive_samples = image_features.shape[0]
        pos_text_features = text_features[:num_positive_samples]  # Positive 텍스트
        neg_text_features = text_features[num_positive_samples:]  # Negative 텍스트

        # Combine Positive and Negative Text Features
        combined_text_features = torch.cat([pos_text_features, neg_text_features], dim=0)

        # Logits Calculation
        logits_per_image = logit_scale * image_features @ combined_text_features.T
        logits_per_text = logit_scale * combined_text_features @ image_features.T

        # Define labels for Positive samples
        labels = torch.arange(num_positive_samples, device=device, dtype=torch.long)

        # Contrastive loss with all Negative pairs
        # Positive pairs (image ↔ positive text)
        pos_loss_image_to_text = F.cross_entropy(
            logits_per_image[:, :num_positive_samples], labels
        )
        pos_loss_text_to_image = F.cross_entropy(
            logits_per_text[:num_positive_samples, :], labels
        )

        # Negative pairs (image ↔ negative text)
        neg_loss_image_to_text = F.cross_entropy(
            logits_per_image[:, num_positive_samples:], labels
        )
        neg_loss_text_to_image = F.cross_entropy(
            logits_per_text[num_positive_samples:, :], labels
        )

        # Combine all losses
        total_contrastive_loss = (
            pos_loss_image_to_text
            + pos_loss_text_to_image
            + neg_loss_image_to_text
            + neg_loss_text_to_image
